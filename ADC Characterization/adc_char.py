...
plt.plot(sampno8k, 10*np.log10(fftaddi), marker='x')
plt.show()
nad=[]
for i in fftaddi:
    nad.append(i)
...
nad[8072]=0
x=np.sum(nad)
sinad=10*np.log10(fftaddi[120]*2/x)
print("SINAD: ", sinad,"dB")
enob=(sinad-1.76)/6.02
print("ENOB: ", enob, "bits")
sfdr=10*np.log10(fftaddi[120]/np.max(nad))
print("SFDR: ", sfdr, "dB")
count=2
while (120*count<4096):
    nad[120*count]=0
    nad[8192-120*count]=0
    count=count+1
x=np.sum(nad)
snr=10*np.log10(fftaddi[120]*2/x)
print("SNR: ", snr, "dB")
